...
class PaperController extends Controller
{
    ...
    public function paperSearch(Request $request){
        ...
        if($request->paper_title&& $request->paper_category){
            $categoryIds = CategoryPaper::where('cat_id',$request->paper_category)->pluck('paper_id');
            $papers = Paper::where('paper_title', 'LIKE',$titleSearch)->orWhereIn('id',$categoryIds)->orderBY('created_at','desc')->paginate(12);
            ...
            $categoryIds = CategoryPaper::where('cat_id',$request->paper_category)->pluck('paper_id');
            $papers = Paper::whereIn('id', $categoryIds)->orderBY('created_at','desc')->paginate(12);
        }else{
            $papers = Paper::orderBY('created_at','desc')->paginate(12);
        }
    ...
    public function categoryWisePaper($cat_meta_data){
        ...
        $categoryIds = CategoryPaper::where('cat_id',$category->id)->pluck('paper_id');
        $papers = Paper::whereIn('id', $categoryIds)->orderBY('created_at','desc')->paginate(12);
        $categories = Category::pluck( 'cat_name', 'id');
        return view('paper.index', compact( 'papers','categories'));
    }
